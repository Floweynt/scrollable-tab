...
public class BetterPingCompatHandler implements ICompatHandler {
    public interface BetterPingCallback {
        void render(Minecraft client, PlayerTabOverlay hud, GuiGraphics context, int width, int x, int y, PlayerInfo player);
    }
            ...
            try {
                var method =
                    Class.forName("com.vladmarica.betterpingdisplay.hud.CustomPlayerListHud")
                        .getMethod(
                            "renderPingDisplay",
                            Minecraft.class, PlayerTabOverlay.class, GuiGraphics.class, int.class, int.class, int.class, PlayerInfo.class
                        );
